      ...
      <div className="flex items-center justify-between px-4 sm:px-6 h-16">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            <button
              onClick={() => { setLangMenu(!langMenu); setNotifOpen(false); setUserMenu(false); }}
              className="flex items-center gap-1.5 p-2 sm:px-3 sm:py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 transition-colors ring-1 ring-slate-200 dark:ring-slate-700 text-xs font-semibold"
              title="Change Language"
            >
              <Globe className="w-4 h-4 text-primary-600 dark:text-primary-400 shrink-0" />
              <span className="hidden xs:inline">{currentLanguageOption.flag}</span>
              <span className="hidden md:inline">{currentLanguageOption.nativeName}</span>
            </button>
              ...
              {langMenu && (
                <>
                  ...
                  <motion.div
                    ...
                    className="absolute right-0 top-full mt-2 w-52 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-xl p-2 z-50"
                  >
          ...
          <div className="relative">
            <button
              onClick={() => { setNotifOpen(!notifOpen); setUserMenu(false); setLangMenu(false); }}
          ...
          <div className="relative">
            <button
              onClick={() => { setUserMenu(!userMenu); setNotifOpen(false); setLangMenu(false); }}
                  ...
                  >
                    ...
                    {profile?.role === 'admin' && (
                      <Link
                        ...
                        className="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 text-sm text-slate-700 dark:text-slate-200"
                      >
                        <Settings className="w-4 h-4 text-primary-500" /> {t('nav.adminTools')}
                      </Link>
                    )}
                    <button
