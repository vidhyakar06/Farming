...
export default function FarmDetails() {
  ...
  useEffect(() => {
    const fetchFarm = async () => {
      ...
      try {
        ...
        if (error) {
          console.warn('Supabase fetch error for farm details:', error.message);
        }

        let record = data as FarmDetail | null;

        // Fallback to local storage cache if database returned no rows or errored
        if (!record) {
          try {
        ...
        if (record) {
          ...
          setForm({
            ...
            soil_ph: record.soil_ph != null ? String(record.soil_ph) : '',
            nitrogen: record.nitrogen != null ? String(record.nitrogen) : '',
            phosphorus: record.phosphorus != null ? String(record.phosphorus) : '',
            potassium: record.potassium != null ? String(record.potassium) : '',
            rainfall: record.rainfall != null ? String(record.rainfall) : '',
          ...
          try {
            localStorage.setItem(`farm_details_${session.user.id}`, JSON.stringify(record));
          } catch {
            // ignore
          }
        }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let savedRecord: FarmDetail | null = null;
      ...
      if (existing?.id) {
        ...
        if (error) {
          console.warn('Update error, fallback to memory/cache:', error.message);
        }
        savedRecord = (data as FarmDetail) || { ...existing, ...payload };
      } else {
        const { data, error } = await supabase
          .from('farm_details')
          .insert(payload)
        ...
        if (error) {
          console.warn('Insert error, fallback to memory/cache:', error.message);
        }
        savedRecord = (data as FarmDetail) || {
          id: 'farm_' + Date.now(),
          ...payload,
  ...
  return (
    <div className="pb-24">
      ...
      {existing && (
        <Card className="p-5 mb-6 border-l-4 border-l-emerald-500 bg-white/90 dark:bg-slate-800/90 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4 pb-3 border-b border-slate-100 dark:border-slate-700/50">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-lg bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center text-emerald-600 dark:text-emerald-400">
                <CheckCircle2 className="w-4 h-4" />
              </div>
              <div>
                <h4 className="font-semibold text-slate-800 dark:text-white text-sm">Active Farm Profile</h4>
                <p className="text-xs text-slate-500 dark:text-slate-400">Current details used for crop and fertilizer suggestions</p>
              </div>
            </div>
            <span className="self-start sm:self-auto text-xs font-semibold px-2.5 py-1 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300">
              Details Saved
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-700/40">
              <span className="text-[10px] font-semibold text-slate-400 block uppercase tracking-wider">Soil & pH</span>
              <p className="font-bold text-slate-800 dark:text-white text-sm mt-0.5">{existing.soil_type || 'N/A'}</p>
              <p className="text-xs text-emerald-600 dark:text-emerald-400 font-medium">pH {existing.soil_ph != null ? existing.soil_ph : 'N/A'}</p>
            </div>
            ...
            <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-700/40">
              <span className="text-[10px] font-semibold text-slate-400 block uppercase tracking-wider">Weather Conditions</span>
              <p className="font-bold text-slate-800 dark:text-white text-sm mt-0.5">{existing.temperature ?? '-'}°C • {existing.humidity ?? '-'}%</p>
              <p className="text-xs text-slate-500 dark:text-slate-400">{existing.rainfall ?? '-'} mm rain</p>
            </div>
